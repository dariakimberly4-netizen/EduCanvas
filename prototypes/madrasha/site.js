(() => {
  ...
  const asatizahGrid = document.querySelector("#public-asatizah-grid");
  if (asatizahGrid && Array.isArray(state.asatizah)) {
    const profiles = state.asatizah.filter(profile => profile.status === "Published");
    asatizahGrid.innerHTML = profiles.map((profile, index) => {
      ...
      const area = /primary|early/.test(searchableArea)
        ? "primary"
        : /science|physics|chemistry|biology|math/.test(searchableArea)
          ? "science"
          : "humanities";
      return `<article data-asatizah-area="${area}">
    ...
    const count = document.querySelector("#asatizah-count");
    if (count) count.textContent = `${profiles.length} members`;
  }

  if (asatizahGrid) {
    const asatizahButtons = [...document.querySelectorAll("[data-asatizah-filter]")];
    const asatizahCount = document.querySelector("#asatizah-count");
    asatizahButtons.forEach(button => button.addEventListener("click", () => {
      const filter = button.dataset.asatizahFilter;
      let visible = 0;
      asatizahButtons.forEach(item => {
        const active = item === button;
        item.classList.toggle("filter-active", active);
        item.setAttribute("aria-pressed", String(active));
      });
      asatizahGrid.querySelectorAll("article").forEach(profile => {
        const show = filter === "all" || profile.dataset.asatizahArea === filter;
        profile.hidden = !show;
        if (show) visible += 1;
      });
      if (asatizahCount) asatizahCount.textContent = `${visible} ${visible === 1 ? "member" : "members"}`;
    }));
  }
