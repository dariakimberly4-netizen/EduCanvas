(() => {
  ...
  const facultyGrid = document.querySelector("#public-faculty-grid");
  if (facultyGrid && Array.isArray(state.faculty)) {
    const profiles = state.faculty.filter(profile => profile.status === "Published");
    facultyGrid.innerHTML = profiles.map((profile, index) => {
      ...
      const area = /primary|early/.test(searchableArea)
        ...
        : /science|physics|chemistry|biology|math/.test(searchableArea)
          ? "science"
          : "humanities";
      return `<article data-faculty-area="${area}">
  ...
  if (facultyGrid) {
    const facultyButtons = [...document.querySelectorAll("[data-faculty-filter]")];
    const facultyCount = document.querySelector("#faculty-count");
    facultyButtons.forEach(button => button.addEventListener("click", () => {
      const filter = button.dataset.facultyFilter;
      let visible = 0;
      ...
      facultyGrid.querySelectorAll("article").forEach(profile => {
        const show = filter === "all" || profile.dataset.facultyArea === filter;
        profile.hidden = !show;
        if (show) visible += 1;
      });
      if (facultyCount) facultyCount.textContent = `${visible} ${visible === 1 ? "member" : "members"}`;
    }));
  }
