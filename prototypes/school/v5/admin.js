(() => {
  ...
  const defaultData = {
    landing: {
      ...
      heroSummary: "Shapla Grove is a co-educational school where strong academics, attentive teachers, and a safe campus help every student progress with confidence.",
      phone: "+880 1712 345 678",
      admissionYear: "2026–27",
      schoolHeading: "A clear standard for learning and care.",
      schoolIntro: "Families choose Shapla Grove for a balanced education: structured teaching, close guidance, and room for students to discover what they do well.",
    ...
    documents: [
      ...
      { id: 102, title: "School closure for Ashura", type: "Notice", category: "General", date: "2026-07-18", fileName: "school-closure.pdf", status: "Published" },
      { id: 103, title: "Inter-house science fair: registration details", type: "Notice", category: "Event", date: "2026-07-12", fileName: "science-fair.pdf", status: "Published" },
  ...
  const loadData = () => {
    try {
      ...
      if (saved && saved.landing && saved.faculty && saved.documents) {
  ...
  const openView = view => {
    ...
    if (currentView === "faculty") renderFaculty();
    if (currentView === "documents") renderDocuments();
  };

  const updateCounts = () => {
    const facultyCount = state.faculty.filter(item => item.status === "Published").length;
    ...
    $$("[data-faculty-count]").forEach(el => el.textContent = facultyCount);
  ...
  const updateLandingPreview = () => {
    ...
    $("#preview-descriptor").textContent = data.schoolDescriptor || "School description";
  ...
  const buildChangeSummary = () => {
    ...
    const labels = {
      ...
      schoolDescriptor: "School description",
      heroTitle: "Main heading",
      heroSummary: "Introduction",
      phone: "Phone number",
      admissionYear: "Admission year",
      schoolHeading: "School story heading",
  ...
  const renderFaculty = () => {
    const search = $("#faculty-search").value.trim().toLowerCase();
    const status = $("#faculty-filter").value;
    const filtered = state.faculty.filter(item => {
      const matchesSearch = `${item.name} ${item.role} ${item.department} ${item.subject}`.toLowerCase().includes(search);
      const matchesStatus = status === "all" || item.status === status;
      return matchesSearch && matchesStatus;
    });
    $("#faculty-visible-count").textContent = filtered.length;
    $("#faculty-empty").hidden = filtered.length > 0;
    $("#faculty-manager-list").innerHTML = filtered.map(item => `
      <article class="faculty-manager-row">
        ...
        <div class="row-actions"><button type="button" data-edit-faculty="${item.id}">Edit</button><button class="danger-link" type="button" data-delete-faculty="${item.id}">Remove</button></div>
      </article>`).join("");
  };
  ...
  const openFacultyDialog = item => {
    const form = $("#faculty-form");
    form.reset();
    form.elements.facultyId.value = item?.id || "";
    ["name", "role", "department", "subject", "experience", "status", "bio"].forEach(key => {
      if (item) form.elements[key].value = item[key] ?? "";
    });
    $("#faculty-dialog-title").textContent = item ? "Edit faculty member" : "Add faculty member";
    $("#save-faculty").textContent = item ? "Save changes" : "Add profile";
  ...
  $("#add-faculty").addEventListener("click", () => openFacultyDialog());
  $("#faculty-search").addEventListener("input", renderFaculty);
  $("#faculty-filter").addEventListener("change", renderFaculty);
  $("#faculty-manager-list").addEventListener("click", event => {
    const editId = Number(event.target.dataset.editFaculty);
    const deleteId = Number(event.target.dataset.deleteFaculty);
    if (editId) openFacultyDialog(state.faculty.find(item => item.id === editId));
    if (deleteId) {
      const item = state.faculty.find(profile => profile.id === deleteId);
      confirmAction(`Remove ${item.name}?`, "This profile will no longer appear on the public Faculty page.", () => {
        state.faculty = state.faculty.filter(profile => profile.id !== deleteId);
        addActivity("Removed faculty profile", item.name);
        save();
        renderFaculty();
        refreshOverview();
  ...
  $("#faculty-form").addEventListener("submit", event => {
    event.preventDefault();
    const form = event.currentTarget;
    if (!form.reportValidity()) return;
    const values = Object.fromEntries(new FormData(form));
    const id = Number(values.facultyId);
    const item = {
      id: id || Date.now(),
    ...
    if (id) state.faculty = state.faculty.map(profile => profile.id === id ? item : profile);
    else state.faculty.unshift(item);
    addActivity(id ? "Updated faculty profile" : "Added faculty profile", item.name);
    save();
    $("#faculty-dialog").close();
    renderFaculty();
    refreshOverview();
    showToast(id ? "Profile updated" : "Profile added", `${item.name} is ${item.status.toLowerCase()} on the Faculty page.`);
  });
  ...
  $("#reset-demo").addEventListener("click", () => {
    confirmAction("Reset all prototype data?", "This restores the original landing page, faculty profiles, documents, and activity.", () => {
