...
export function AdminDocuments({ content, updateContent, notify }: AdminContentEditorProps) {
  ...
  async function saveDocument(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      notify("Document publishing failed", getErrorMessage(error, "The document could not be published."));
    } finally {
  ...
  async function removeDocument() {
    ...
    try {
      ...
      notify("Document removal failed", getErrorMessage(error, "The document could not be removed."));
    }
  }
  ...
  return (
    <>
          ...
          <div>
            {documents.map((document) => (
              <article className="document-manager-row" key={document.id}>
                ...
                <span>{formatShortDate(document.date)}</span><span><i className="status-chip published">{document.status}</i></span>
      ...
      <ConfirmationDialog
        ...
        description={<>{pendingDelete?.title} will no longer appear on the Notices &amp; results page, and its PDF will be deleted from Google Drive. This action cannot be undone.</>}
        confirmLabel="Remove"
