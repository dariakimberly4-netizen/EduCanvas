...
export function AdminOverview({ content, openView }: AdminOverviewProps) {
  ...
  const landingPublished = Boolean(content.landing.publishedAt.trim());
  const landingPublishedLabel = landingPublished
    ? content.landing.publishedAt
    : "Not published yet";

  return (
    <>
      <div className="admin-welcome">
        <div><p className="overline">Website overview</p><h2 id="overview-title">Everything families see, managed in one place.</h2><p>Update the school landing page, maintain faculty profiles, and publish notice or result PDFs. Published changes are stored centrally and appear on the public website immediately.</p></div>
      ...
      <section className="admin-health">
        <div className="admin-section-title"><div><p className="overline">Website status</p><h2>Published content</h2></div><p>{landingPublished ? `Landing page published ${landingPublishedLabel}` : landingPublishedLabel}</p></div>
        <div className="admin-summary">
          ...
          <article><span>Landing page</span><strong className="text-status">{landingPublished ? "Published" : "Not published"}</strong><small><i />{landingPublished ? landingPublishedLabel : "No public version"}</small></article>
        </div>
      </section>
